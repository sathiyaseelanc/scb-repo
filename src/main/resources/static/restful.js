  ...
  function callAjax(idToken){
	//multiGet();
	 // alert(idToken);
	  //$.get('/api/rest/patient-data', function(data) {
	  $.ajax({
         // dataType: 'json',
          headers: {
              'ID-TOKEN': ''+idToken
          },
          url: '/api/rest/customer-data',
          type: 'GET',
         /* beforeSend: function (jqXHR, settings) {
              jqXHR.setRequestHeader('Authorization', 'Basic ' + btoa('myuser:mypswd'));
          },*/
          //beforeSend: function(xhr){xhr.setRequestHeader('ID-TOKEN': idToken);},
